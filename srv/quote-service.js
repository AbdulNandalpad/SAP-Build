...
async function fetchAllQuotes(c4c, req) {
...
module.exports = cds.service.impl(async function () {
    ...
    this.on('READ', 'SalesQuotes', async (req) => {
        try {
            ...
            const probe = await c4c.tx(req).send({ method: 'GET', path: 'SalesQuoteCollection?$top=1&$select=ObjectID,ID,Name' });
            console.log('[CAP] SalesQuoteCollection probe OK, records:', JSON.stringify(probe).substring(0, 200));
            if (top && top <= 100) {
                const result = await c4c.tx(req).send({ method: 'GET', path: 'SalesQuoteCollection?$top=' + top + '&$select=' + QUOTE_SEL });
                return Array.isArray(result) ? result : (result.value || result);
            }
            return await fetchAllQuotes(c4c, req);
        } catch(e) {
